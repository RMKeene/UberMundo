...
UCLASS()
class SHARECLIENTPC_API UShareRequest : public UObject {
	GENERATED_BODY()
public:
...
UCLASS()
class SHARECLIENTPC_API UShareGetBlockState : public UShareRequest {
	GENERATED_BODY()
public:
...
UCLASS()
class SHARECLIENTPC_API USharePutBlockState : public UShareRequest {
	GENERATED_BODY()
public:
 ...
 */
UCLASS()
class SHARECLIENTPC_API UBlockDataClient : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Share Asset IO", meta = (ToolTip = "Start a request in the background to fetch a Share Block state as a string."))
		static void RequestShareBlock(FString blockPathAndName, int64& requestHandle, bool& success);
	UFUNCTION(BlueprintCallable, Category = "Share Asset IO", meta = (ToolTip = "Start a request in the background to put a Share Block state as a string."))
		static void WriteShareBlock(FString blockPathAndName, int64& requestHandle, bool& success, FString contents);
	UFUNCTION(BlueprintCallable, Category = "Share Asset IO")
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Share Asset IO")
		static void GetShareBlockRequestStatus(int64 requestHandle, TEnumAsByte<SharedRequestStatus>& status, FString& failReason);
	UFUNCTION(BlueprintCallable, Category = "Share Asset IO", meta = (ToolTip = "Once Get Share Block Request Status returns Success you can call this to get the data."))
	...
	UFUNCTION(BlueprintCallable, Category = "Share Asset Helpers", meta = (ToolTip = "List the Blueprint CLasses."))
		static bool ListAllBlueprintsInPath(FName Path, TArray<UClass*>& Result, UClass* Class);
	UFUNCTION(BlueprintCallable, Category = "Share Asset Helpers", meta = (ToolTip = "List the assets. Each string is type then path like \"Blueprint /Game/DefaultObjects/BigBox.BigBox\"/"))
